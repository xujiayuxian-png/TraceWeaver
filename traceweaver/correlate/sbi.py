...
def _connection_key(record) -> str:
    tcp_stream = _parse_optional_int(record.fields.get("tcp.stream"))
...
def pair_sbi_calls(record_set: ExtractedRecordSet) -> list[SBICall]:
    ...
    for record in record_set.records:
        if record.primary_protocol != "http2":
            continue
        stream_id = _parse_optional_int(record.fields.get("http2.streamid"))
        if stream_id is None:
            continue
        path = _first_non_empty(record.fields.get("http2.headers.path"))
        status = _parse_optional_int(record.fields.get("http2.headers.status"))
    ...
    for (connection_key, stream_id), records in sorted(grouped.items(), key=lambda item: (item[0][0], item[0][1])):
        ...
        response_record = next(
            (record for record in records if _parse_optional_int(record.fields.get("http2.headers.status")) is not None),
            None,
        )
        ...
        status = next(
            (
                parsed
                for parsed in (_parse_optional_int(record.fields.get("http2.headers.status")) for record in records)
        ...
        call = SBICall(
            ...
            tcp_stream=_parse_optional_int(seed_record.fields.get("tcp.stream")),
            stream_id=stream_id,
...
def correlate_sbi_to_sessions(calls: list[SBICall], sessions: list[UESession]) -> list[SBICall]:
